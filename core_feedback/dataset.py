...
class TaskDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        result['signal2_strength'] = torch.as_tensor(self.trial.signal2_strength)
        result['feedback_threshold_mask'] = torch.as_tensor(self.trial.feedback_threshold_mask)

        result['epochs'] = self.trial.epochs

        if self.rule_name == 'spatial_reproduction':
