...
class TrainStepper(object):
    ...
    def __init__(self, model, hp, is_cuda=False):
        ...
        self.l2_feedback_current = torch.tensor(hp['l2_feedback_current'], device=self.device)
        #self.l1_feedback_current = torch.tensor(hp['l1_feedback_current'], device=self.device)
        ...
        self.l2_feedback_current_cpu = torch.tensor(hp['l2_feedback_current'], device=torch.device("cpu"))
        #self.l1_feedback_current_cpu = torch.tensor(hp['l1_feedback_current'], device=torch.device("cpu"))
    ...
    def cost_fcn(self, **kwargs):
        ...
        feedback_threshold_mask = kwargs['feedback_threshold_mask']
        signal2_strength = kwargs['signal2_strength']

        #print(cost_mask.device, feedback_threshold_mask.device)
        ...
        self.input_size = self.rnn_net.input_size
        self.input_feedback_size = self.rnn_net.input_feedback_size
        self.sensory_threshold = self.rnn_net.sensory_threshold

        self.state_collector, self.feedback_current_collector = self.forward(inputs, initial_state)
        ...
        self.feedback_current_binder = torch.cat(self.feedback_current_collector[cost_start_time:cost_end_time], dim=0).view(-1, self.batch_size, self.input_feedback_size)
        ...
        feedback_threshold_mask_length = torch.sum(feedback_threshold_mask, dim=0)


        self.cost_feedback = torch.mean(
            torch.sum((nn.functional.relu(nn.functional.relu(self.sensory_threshold - signal2_strength) - self.feedback_current_binder) ** self._2) * feedback_threshold_mask, dim=0) / feedback_threshold_mask_length)
        ...
        if self.l2_feedback_current_cpu > 0:
            #print('hello')
            seq_mask_n_element = torch.sum(seq_mask, dim=0)
            self.cost_reg = self.cost_reg + torch.mean(torch.sum((self.feedback_current_binder * seq_mask) ** self._2,
                                                                 dim=0) / seq_mask_n_element) * self.l2_feedback_current

        self.cost = self.cost_lsq + self.cost_feedback + self.cost_reg
    ...
    def stepper(self, **kwargs):
        ...
        self.rnn_net.feedback_weight_clipper()
