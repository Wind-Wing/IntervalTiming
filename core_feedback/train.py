...
def get_perf_spatial_reproduction(output, target_dir, fix_start, fix_end, fix_strength=0.2, action_threshold=0.5, response_duration=int(300/20), n_gaussianline=32.):
    ...
    def do_eval(self):
        ...
        for i_batch, sample_batched in enumerate(self.dataloader_test):
            ...
            if self.is_cuda:
                ...
                sample_batched['feedback_threshold_mask'] = sample_batched['feedback_threshold_mask'].cuda()
                sample_batched['signal2_strength'] = sample_batched['signal2_strength'].cuda()
            ...
            # save performance
            if self.rule_name == 'ctx_decision_making':
                ...
                success_action_prob,  mean_direction_err = get_perf_spatial_reproduction(self.train_stepper.outputs.detach().cpu().numpy(), sample_batched['gaussian_center'], sample_batched['epochs']['stim1'][1], sample_batched['epochs']['go_cue'][1], n_gaussianline=32.)
                success_action_prob = success_action_prob.tolist()
